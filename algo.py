...
def tier_cascade_sort_visual_full(data, map_grootte=6, laag_pct=0.3, mid_pct=0.4, hoog_pct=0.3):
    ...
    buffer = []
    snapshots = []  # opslaan van stappen voor animatie
    buffers_snapshots = []  # ook buffer voor laten zien
    
    for getal in data:
        buffer.append(getal)
        buffers_snapshots.append(buffer[:])  # sla huidige buffer op
    ...
    # Maak cumulatieve snapshots van eindresultaat (voegt mappen samen stap voor stap)
    for m in kleine_mappen + midden_mappen + grote_mappen:
        alles.extend(m)
        cumulatief_snapshots.append(sorted(alles[:]))  # Altijd gesorteerd tussendoor toevoegen

    # Final sort pass (insertion sort)
    def is_sorted(lst):
    ...
    if not is_sorted(alles):
        for i in range(1, len(alles)):
            ...
            while j >= 0 and alles[j] > key:
                ...
                j -= 1
            alles[j + 1] = key
        cumulatief_snapshots.append(alles[:])  # voeg definitief gesorteerde lijst toe
...
def animate_full_process(snapshots, buffers_snapshots, cumulatief_snapshots):
    colors = ['#3498db', '#2ecc71', '#e74c3c']  # blauw, groen, rood
    labels = ['Kleine map', 'Midden map', 'Grote map']
    ...
    def draw_split(klein, midden, groot, step):
        ...
        ax_top.set_ylim(0, 1)
        ax_top.axis('off')
        ax_top.set_title(f'Splitsing stap {step + 1} van {len(snapshots)}')
    ...
    def draw_cumulatief(alles, step, final=False):
        ...
        ax_bottom.set_ylim(0, 1)
        ax_bottom.axis('off')
        titel = f'Cumulatief eindresultaat stap {step + 1} van {len(cumulatief_snapshots)}'
        if final:
            titel += " (Volledig gesorteerd)"
    ...
    def update(frame):
        if frame < len(buffers_snapshots):
            ...
            ax_bottom.clear()
            ax_bottom.axis('off')
            ax_bottom.set_title('Cumulatief eindresultaat')
...
# Voorbeeld data en animatie starten
random.seed(420)
...
print("Gesorteerde lijst:", gesorteerd)
